...
export default function Index({ iurans, filters }) {
    ...
    return (
        <WargaLayout header="Riwayat Iuran Saya">
                            ...
                            {iurans.data && iurans.data.length > 0 ? (
                                iurans.data.map((item) => (
                                    <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-4 text-sm text-gray-900 font-medium">{bulanNames[item.periode_bulan]} {item.periode_tahun}</td>
                                        <td className="px-6 py-4 text-sm text-gray-500">{item.jenis_iuran}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900 font-medium">Rp {Number(item.jumlah_bayar).toLocaleString('id-ID')}</td>
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColor(item.status_pembayaran)}`}>{item.status_pembayaran}</span>
                                        </td>
                                    </tr>
                                ))
                            ) : (
                                <tr><td colSpan="4" className="px-6 py-10 text-center text-gray-500">Belum ada data iuran.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
