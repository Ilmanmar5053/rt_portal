...
export default function Show({ keluarga }) {
    ...
    return (
        ...
        >
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    className="px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-amber-300 font-bold text-xs transition border border-white/10 flex items-center gap-1 cursor-pointer"
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500 font-medium">
                                    Klik <b>Detail</b> untuk menampilkan card ID 4x5 warga.
                                </p>
                            </div>
                        </div>
                                            ...
                                            <td className="px-4 py-3 text-right text-xs">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <button
                                                        ...
                                                        title="Lihat Card Detail Warga 4x5"
                                                    >
                                                        <span>👁️</span>
                                                        <span>Detail</span>
                                                    </button>
            ...
            {/* ULTRA COMPACT 4x5 ID CARD POPUP (TANPA TIMER AUTO-HIDE) */}
            {selectedWargaDetail && (
                <div 
                    onClick={() => setSelectedWargaDetail(null)}
                    className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-3 animate-in fade-in duration-150"
                >
                    <div 
                        onClick={(e) => e.stopPropagation()}
                        className="bg-white rounded-2xl shadow-2xl w-[340px] overflow-hidden border-2 border-indigo-500/30 animate-in zoom-in-95 duration-150 relative"
                    >
                            ...
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <h4 className="font-black text-xs text-white uppercase tracking-tight truncate max-w-[200px]">
                                        {selectedWargaDetail.nama_lengkap}
                                    </h4>
                                </div>
                            </div>
                            <button
                                onClick={() => setSelectedWargaDetail(null)}
                                className="w-6 h-6 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white text-xs font-bold transition cursor-pointer"
                            >
                                ✕
                            </button>
                        </div>
                        ...
                        <div className="p-3.5 space-y-2.5 text-[11px]">
                            ...
                            <div className="p-2 bg-indigo-50/80 rounded-xl border border-indigo-100 flex items-center justify-between">
                                <div>
                                    ...
                                    <span className="font-mono font-black text-xs text-indigo-950">{selectedWargaDetail.nik}</span>
                                </div>
                                <span className={`px-2 py-0.5 rounded-md text-[10px] uppercase font-black ${getStatusBadge(selectedWargaDetail.status_hubungan_keluarga)}`}>
                                    {selectedWargaDetail.status_hubungan_keluarga}
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 text-[10.5px]">
                                <div className="p-2 bg-slate-50 rounded-lg">
                                    ...
                                    <span className="font-extrabold text-slate-800">{selectedWargaDetail.jenis_kelamin || 'Laki-laki'}</span>
                                </div>
                                ...
                                <div className="p-2 bg-slate-50 rounded-lg">
                                    ...
                                    <span className="font-extrabold text-slate-800 truncate block">
                                        {selectedWargaDetail.tempat_lahir || 'SERANG'}, {selectedWargaDetail.tanggal_lahir || '-'}
                                    </span>
                                </div>

                                <div className="p-2 bg-slate-50 rounded-lg">
                                    <span className="text-[9px] text-slate-400 font-bold block">Pendidikan</span>
                                    <span className="font-extrabold text-slate-800 truncate block">{selectedWargaDetail.pendidikan || '-'}</span>
                                </div>

                                <div className="p-2 bg-slate-50 rounded-lg">
                                    <span className="text-[9px] text-slate-400 font-bold block">Pekerjaan</span>
                                    <span className="font-extrabold text-slate-800 truncate block">{selectedWargaDetail.pekerjaan || '-'}</span>
                                </div>

                                <div className="p-2 bg-slate-50 rounded-lg">
                                    <span className="text-[9px] text-slate-400 font-bold block">Nama Ayah</span>
                                    <span className="font-extrabold text-slate-800 truncate block">{selectedWargaDetail.nama_ayah || '-'}</span>
                                </div>

                                <div className="p-2 bg-slate-50 rounded-lg">
                                    <span className="text-[9px] text-slate-400 font-bold block">Nama Ibu</span>
                                    <span className="font-extrabold text-slate-800 truncate block">{selectedWargaDetail.nama_ibu || '-'}</span>
                                </div>
                            </div>
                            ...
                            <div className="pt-1.5 flex items-center justify-between border-t border-slate-100 text-[10px] font-semibold text-slate-500">
                                <div>📱 <span className="font-bold text-slate-800">{selectedWargaDetail.no_hp || '-'}</span></div>
                                <div>🟢 Status: <span className="font-bold text-emerald-600">{selectedWargaDetail.status_hidup || 'Hidup'}</span></div>
                            </div>
                        </div>
                        ...
                        <div className="px-3 py-2 bg-slate-100 border-t border-slate-200 flex justify-end">
                            <button
                                type="button"
                                onClick={() => setSelectedWargaDetail(null)}
                                className="px-3.5 py-1 rounded-lg bg-slate-900 hover:bg-slate-800 text-white font-bold text-[10px] shadow-xs cursor-pointer"
                            >
                                Tutup
                            </button>
                        </div>
