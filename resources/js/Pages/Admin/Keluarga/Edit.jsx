...
export default function Edit({ keluarga, rumahBloks = [], wargas }) {
    ...
    const currentRumahObj = (rumahBloks || []).find(r => 
        String(r.blok).toUpperCase() === String(initialBlok).toUpperCase() &&
        String(r.nomor_rumah).toUpperCase() === String(initialNoRumah).toUpperCase()
    );

    const [selectedRumahId, setSelectedRumahId] = useState(currentRumahObj ? String(currentRumahObj.id) : '');
    const [isManualInput, setIsManualInput] = useState(!currentRumahObj);
                        ...
                        <div className="space-y-3 pt-2">
                            ...
                            {(isManualInput || (!selectedRumahId && (!data.blok || !data.nomor_rumah))) ? (
                                ...
                                <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 text-emerald-950 font-bold text-xs flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <span>✅</span>
                                        <span>Rumah Terpilih: <strong>Blok {data.blok} No. {data.nomor_rumah}</strong> (Status: Terhubung / Tersedia)</span>
                                    </div>
                                    <button
                                        type="button"
