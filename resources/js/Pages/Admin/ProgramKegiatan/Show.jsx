...
export default function Show({ program }) {
    ...
    const handleDelete = () => {
        if (confirm(`Apakah Anda yakin ingin menghapus program "${program.nama_program}"?`)) {
            router.delete(route('admin.program-kegiatan.destroy', program.id));
        }
    };
    ...
    return (
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <Link
                            ...
                            className="px-4 py-2 rounded-xl font-bold text-sm text-white bg-amber-500 hover:bg-amber-600 shadow-sm transition"
                        >
                            Edit
                        </Link>
                        <button
                            ...
                            className="px-4 py-2 rounded-xl font-bold text-sm text-white bg-rose-600 hover:bg-rose-700 shadow-sm transition"
                        >
                            Hapus
                        </button>
                    </div>
        ...
        >
            <Head title={`${program.nama_program} - Admin`} />
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    {program.pic_kontak && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-sm transition"
                                        >
                                            Chat WA
                                        </a>
                                    )}
                            ...
                            <div className="p-4 rounded-2xl bg-gray-50 border border-gray-200/70">
                                ...
                                <div className="space-y-1">
                                    <div className="flex justify-between text-xs">
                                        ...
                                        <span className="font-extrabold text-gray-900">{formatCurrency(program.estimasi_anggaran)}</span>
                                    </div>
                                    <div className="flex justify-between text-xs">
                                        <span className="text-gray-600">Realisasi:</span>
                                        <span className="font-extrabold text-emerald-700">{formatCurrency(program.realisasi_anggaran)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
